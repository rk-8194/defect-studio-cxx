...
class TetrahedralInterstitial : public Command
{
  ...
  private:
    vector<int> targetAtoms;
    vector<Vector3D> sites;

    void getTargetAtomIndex(CommandArguments &args);
    void generateFirstSite(CommandArguments &args, const int &targetAtomInde);
    void generateSites(CommandArguments &args, const int &targetAtomInde);
